...
while True:
    ...
    for i in range(len(temperature)):
        if len(wind_speed) <= 0:
            print(c(f"Giorno: {days_forecast[i]}, Temperatura: {temperature[i]}°C, Velocità del vento: N/A, Precipitazioni: {precipitation[i]} mm", "red"))
        elif len(precipitation) <= 0:
            print(c(f"Giorno: {days_forecast[i]}, Temperatura: {temperature[i]}°C, Velocità del vento: {wind_speed[i]} km/h, Precipitazioni: N/A", "red"))
        else:
            print(f"Giorno: {c(days_forecast[i], (255, 0, 255))}, Temperatura: {c(temperature[i], (0, 255, 0))}°C, Velocità del vento: {wind_speed[i]} km/h, Precipitazioni: {c(precipitation[i], (0, 0, 255))} mm")